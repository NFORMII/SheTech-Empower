...
const Welcome: React.FC = () => {
  ...
  const renderStep = () => {
    switch (step) {
      case OnboardingStep.LANGUAGE:
        return <LanguageSelector onSelectLanguage={handleLanguageSelect} selectedLanguage={language} />;
      case OnboardingStep.ROLE:
        return <RoleSelector onSelectRole={handleRoleSelect} selectedRole={role} />;
        ...
        return <div className="w-full max-w-md space-y-6">
            <div className="text-center">
              <h2 className="text-2xl font-montserrat font-bold text-gray-800">
                ...
                {authType === 'login' ? t('auth.login_title') : t('auth.signup_title')}
              </h2>
              <p className="mt-2 text-gray-600">
                {authType === 'login' ? t('auth.login_subtitle') : t('auth.signup_subtitle')}
            ...
            <div className="text-center">
              <button onClick={() => setAuthType(authType === 'login' ? 'signup' : 'login')} className="text-primary hover:underline">
                {authType === 'login'
  ...
  return <div className="min-h-screen bg-gradient-to-b from-secondary to-white flex flex-col">
      {/* Header with logo */}
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-6 md:p-8 mb-6">
          ...
          <div className="flex justify-center mb-6">
            <img src="https://images.unsplash.com/photo-1516585427167-9f4af9627e6c?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1000&q=80" alt="Women supporting each other" className="w-full h-40 object-cover rounded-lg" />
          </div>
        ...
        {/* Back button */}
        {step !== OnboardingStep.LANGUAGE && <Button variant="outline" onClick={handleBackClick}>
            Back
          </Button>}
      </main>
      {/* Footer */}
